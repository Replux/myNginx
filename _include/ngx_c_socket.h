...
#define NGX_MAX_EVENTS      512    //epoll_wait单次最多接收的事件数
...
{		
	...
	void Recondition();                                      //分配出去的时候初始化一些内容

	int                    fd;                            //套接字句柄socket
	lpngx_listening_t      listening;                     //若这个连接被分配给了一个监听套接字，那么该成员指向监听套接字对应的那个lpngx_listening_t的内存首地址		
	...
	//和epoll事件有关
	uint32_t               events;                         //记录连接对应socket的已添加至epoll的事件
	
	//和收包有关
	unsigned char          curStat;                        //当前收包的状态
	char                   dataHeadInfo[PKG_HD_BUF];   //用于保存收到的数据的包头信息			
...
public:
	...
	virtual bool Initialize_Worker();                                    //初始化函数[子进程中执行]
	virtual void Shutdown_Worker();                                      //关闭退出函数[子进程中执行]
...
private:	
	bool OpenListeningSocket();                                    //监听必须的端口【支持多个端口】
	void CloseListeningSocket();                                   //关闭监听套接字
	...
	void MessageHandler_p1(lpngx_connection_t pConn,bool &isflood);       //处理客户端发来的消息head      
	void MessageHandler_p2(lpngx_connection_t pConn,bool &isflood);       //处理客户端发来的消息body
	...
	//_________________以后可能会用到的函数_________________
...
private:
	struct ThreadItem   
    {
        ...
        bool        ifReady;                                              //标记是否正式启动起来，启动起来后，才允许调用StopAll()来释放

        //构造函数
        ThreadItem(CSocket *pthis):_pThis(pthis),ifReady(false){}                             
        //析构函数
        ~ThreadItem(){}        
    };
	...
	int                            m_conf_RecyConnWaitTime;              //等待这么些秒后才回收连接
	...
	time_t                         m_earliestTime;                        //当前计时队列头部时间值
