...
private:
    ...
    struct ThreadItem   
    {
        ...
        CThreadPool *_pThis;                        //指向所在线程池	
        bool        ifReady;                      //标记是否正式启动起来，启动起来后，才允许调用StopAll()来释放

        //构造函数
        ThreadItem(CThreadPool *pthis):_pThis(pthis),ifReady(false){}                             
        //析构函数
        ~ThreadItem(){}        
    };

private:
    static pthread_mutex_t     m_threadMutex;      //线程互斥量锁
    ...
    std::vector<ThreadItem *>  m_threadVector;      //容器里就是各个线程了 
