...
void ngx_setproctitle(const char *title)
{
    ...
    char *ptmp = g_argv[0]; //让ptmp指向g_argv所指向的内存
    strcpy(ptmp,title);
    ptmp += titleLength; //跳过标题

    //(5)把剩余的原argv以及environ所占的内存全部清0，否则会出现在ps的cmd列可能还会残余一些没有被覆盖的内容；
    size_t cha = esy - titleLength;  //内存总和减去标题字符串长度(不含字符串末尾的\0)，剩余的大小，就是要memset的；
    memset(ptmp,0,cha);
    return;
}